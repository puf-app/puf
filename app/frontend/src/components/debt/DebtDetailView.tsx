...
import { patchToApi } from '@/lib/api/client';
...
export default function DebtDetailView({ debt, currentUserId }: DebtDetailViewProps) {
  ...
  const handleSettle = async () => {
    ...
    setLoading(true);
    try {
      await patchToApi(`/api/debts/completeDebt/${debt._id}`, {});
  ...
  const handleUpdate = async () => {
    setLoading(true);
    try {
      await patchToApi(`/api/debts/updateDebt/${debt._id}`, {
        title: editTitle,
            ...
            <div className='flex flex-col sm:flex-row gap-4 pt-8 border-t border-gray-100'>
              {isEditing ? (
                ...
                <>
                  ...
                  >
                    <HugeiconsIcon icon={Tick01Icon} size={20} />
                    {loading ? 'Processing...' : 'Settle Debt'}
                  </Button>
                  <Button 
                    variant='outline' 
